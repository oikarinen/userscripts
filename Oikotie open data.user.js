...
function comparePriceAverage() {
    ...
    var avg = avgprices[postalcode][type];
...
function compareCosts() {
    ...
    var hvval = Math.round(parseFloat(hv.innerText.replace(/,/,"."))/m2val*10)/10;
    var rvval = Math.round(parseFloat(rv.innerText.replace(/,/,"."))/m2val*10)/10;
    var age;
    if (by) {
        age = (new Date()).getFullYear() - parseInt(by.innerText);
    }
    // el not used yet
    ...
    hv.innerHTML += " hv "+ hvval + " e/m<sup>2</sup>";
    if (elheat) {
    ...
    rv.innerHTML += " rv "+ rvval + " e/m<sup>2</sup>";
